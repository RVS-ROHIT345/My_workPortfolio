...
`;
    ...
    // Fix for "Back to Top": Re-check visibility/status on scroll and focus
    const handleForceResume = () => {
      if (!webglRef.current) return;
      // If we are at the top and the tab is active, ensure we are running
      if (window.scrollY < 100 && !document.hidden) {
        webglRef.current.start();
      }
    };
    window.addEventListener('scroll', handleForceResume, { passive: true });
    window.addEventListener('focus', handleForceResume);
    ...
    return () => {
      window.removeEventListener('scroll', handleForceResume);
      window.removeEventListener('focus', handleForceResume);
